...
9. As a PoE player, I want the app to tell me which Concept it couldn't find and suggest alternatives, so that I can recover without starting over.
...
**Concept Extractor**
A dedicated LLM agent with a pure interface: `(query, conversation) → {concept, domains[]}[]`. Responsible for resolving anaphoric references against Conversation history and returning a structured list of Concepts, each annotated with the Domains it touches (e.g. `mechanics`, `items`, `market`). Domain annotation drives deterministic routing downstream — this is the only place that reasons about which Sources are relevant. Uses the cheapest available model via OpenRouter — extraction is a classification task, not a reasoning task. Testable in isolation with PoE query fixtures.

**Orchestrator**
Coordinates the full pipeline: receives Concepts from the Concept Extractor, routes them to registered Source Agents via the Source Agent Registry, collects Findings, and synthesizes an Answer. Streams pipeline steps (Concepts identified, Articles fetched) and the final Answer back to the frontend. Never parses raw Query text. Handles out-of-scope detection before invoking the Concept Extractor — if the Query is out of scope, returns a random refusal from the Refusal Pool without any LLM calls.
...
**MediaWiki Client**
Thin, stateless wrapper around the poewiki.net MediaWiki API. Two operations: `search(concept) → title[]` and `fetch(title) → content`. Knows nothing about caching, LLMs, or Source Agents. Always called through the Article Cache, never directly. The optimal content format (wikitext, HTML, or plain text extract) is to be determined by spiking against real poewiki pages before committing to an implementation.
...
2. Orchestrator checks if Query is in scope — if not, returns a refusal immediately
3. Concept Extractor receives Query + Conversation, returns `concepts[]`
...
- **Concept Extractor** — two test layers: (1) CI fixtures using recorded LLM responses — fast, deterministic, free; assert `{concept, domains[]}[]` shape, anaphoric resolution, and domain annotation for known Queries. (2) A manual eval suite (`npm run eval`) that runs against the real model on demand — run before any prompt change to catch annotation regressions. The eval suite covers tricky cases: vague Concepts, cross-domain Queries, anaphoric follow-ups.
