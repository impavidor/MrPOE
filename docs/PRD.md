...
2. As a PoE player, I want the Answer to stream in progressively, so that I don't stare at a blank screen waiting.
3. As a PoE player, I want to ask follow-up questions without restating the full context, so that I can have a natural back-and-forth conversation.
4. As a PoE player, I want follow-up questions that reference "that" or "it" to be resolved correctly, so that the app understands what I mean.
5. As a PoE player, I want my Conversation to persist across page refreshes, so that I don't lose context when I accidentally close the tab.
6. As a PoE player, I want the app to still answer what it can when only some Concepts are missing, so that I get a partial answer rather than nothing.
7. As a PoE player, I want the app to tell me which Concept it couldn't find and ask me to clarify, so that I can recover without starting over.
8. As a PoE player, I want the app to ask for rephrasing when it can't find anything at all, so that I know the full question failed and why.
9. As a PoE player, I want the app to refuse off-topic questions with a PoE-flavored message, so that the experience stays fun even when I ask something out of scope.
10. As a PoE player, I want answers that only cover what I asked about, so that I'm not overwhelmed by irrelevant mechanic details from the same wiki page.
11. As a PoE player, I want the app to handle questions that span multiple Concepts (e.g. crit and ailments), so that I don't have to ask two separate questions.
12. As a PoE player, I want the app to be fast on repeat questions, so that cached answers feel instant.
13. As a PoE player, I want the interface to feel like a chat conversation, so that asking questions feels natural rather than like filling out a search form.
...
**Orchestrator**
Coordinates the full pipeline: invokes the Concept Extractor, inspects its result, and if `outOfScope: true`, returns a random refusal from the Refusal Pool immediately. Otherwise routes Concepts to registered Source Agents via the Source Agent Registry, collects Findings, and synthesizes an Answer. Streams the Answer back to the frontend. Never parses raw Query text.
...
**Extraction Cache**
Wraps Upstash Redis. Interface: `get(concept, title) / set(concept, title, sections)`. Cache key is `{concept}:{article_title}`. Cache value is the extracted sections for that specific (Concept, Article) pair — not the raw Article content. TTL is 2 weeks — PoE 1 wiki content changes only after patches. The only caching layer; there is no Article cache and no Answer cache.

On a cache hit the poewiki Source Agent skips both the Article fetch and the LLM extraction call — the extraction result is returned directly. On a cache miss, the Agent fetches the Article from MediaWiki and runs the extraction call, then caches the result before returning.

Extractions that complete before a Vercel timeout are still cached. This means a cold query that times out partially warms the cache — a retry is likely to succeed with fewer LLM calls.

**MediaWiki Client**
Thin, stateless wrapper around the poewiki.net MediaWiki API. Two operations: `search(concept) → title[]` and `fetch(title) → content`. Knows nothing about caching, LLMs, or Source Agents. Always called through the Article Cache, never directly. Two things are to be determined by spiking against real poewiki pages before committing to an implementation: (1) the optimal Article content format (wikitext, HTML, or plain text extract) — see SPIKE-001; (2) the optimal search endpoint (`action=opensearch` vs `action=query&list=search`) — see SPIKE-002.

**Rate Limiter**
Protects the backend from abuse and uncontrolled LLM costs. Applied at the Vercel API route level before any LLM call is made. Implemented with `@upstash/ratelimit` on the same Upstash Redis instance as the Article Cache — no additional infrastructure. Rate limiting is per IP address (no user accounts exist). Limits are configurable constants (requests per window and window duration) to be tuned based on expected traffic and cost tolerance. When a request is rejected, the API returns HTTP 429 and the frontend shows a friendly message. The OpenRouter API key is a server-side Vercel environment variable — never exposed to the client. If Upstash is unavailable when the rate limiter runs, fail open — allow the request through rather than blocking all users during a Redis outage.

**Infrastructure failure behaviour**
- **Upstash unavailable (Extraction Cache)** — fall through to a direct MediaWiki fetch and live LLM extraction. The cache is an optimisation; the pipeline still works without it, just slower.
- **MediaWiki API unavailable** — treat as total failure (no Articles can be fetched). Return a PoE-flavored error message from a small static pool (same style as the Refusal Pool). No LLM call is made.
- **OpenRouter / LLM unavailable** — return a generic friendly error message. No PoE flavoring — this is an infrastructure failure, not a domain one.
...
**Chat UI**
React frontend using Vercel AI SDK's `useChat` hook. Renders a scrolling chat thread of Query/Answer pairs. Answer streams in progressively as the Orchestrator synthesizes it. No pipeline transparency — Concepts and Articles are internal implementation details not surfaced in the UI.

### Architecture

- Frontend: React 19 + TypeScript, deployed to Vercel
- Backend: Vercel serverless functions (API routes)
- LLM: OpenRouter via Vercel AI SDK — cheapest capable model for Concept Extractor and extraction calls, slightly better model for Orchestrator synthesis
- Cache: Upstash Redis (Article Cache only)
- Source Agent tooling: Vercel AI SDK `tool()` calls — not MCP servers

### Pipeline per turn

1. Frontend sends Query + full Conversation (Q/A pairs) to the Vercel API route
2. API route extracts prior Queries from the Conversation (strips Answers); Concept Extractor receives Query + prior Queries only, returns `{outOfScope: true} | {concepts: [...]}`
3. If `outOfScope`, Orchestrator returns a random refusal from the Refusal Pool immediately — no further LLM calls
4. Source Agent Registry routes Concepts to the poewiki Source Agent
5. poewiki Source Agent queries MediaWiki search API per Concept (parallel), fetches all Articles in parallel via Article Cache, runs one parallel extraction LLM call per Article, concatenates extractions into one Finding
6. Orchestrator synthesizes Finding + Conversation into Answer, streams to frontend
7. Frontend appends Query/Answer pair to Conversation, saves to localStorage

### Caching

| Layer | Key | Value | TTL |
|-------|-----|-------|-----|
| Extraction Cache | `{concept}:{article_title}` | Extracted sections for that Concept/Article pair | 2 weeks |

Raw Article content is never cached — only the extraction output. A cold query that times out still caches any extractions that completed, so retries converge quickly.
...
- Pipeline transparency (surfacing Concepts or Articles in the UI)
